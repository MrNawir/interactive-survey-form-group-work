...
function isEmailValid(email) {
  const pattern = /\S+@\S+\.\S+/; // not perfect, but good enough here
...
form.addEventListener('submit', function (event) {
  ...
  const recommendationRadio = document.querySelector('input[name="recommendation"]:checked');
  const recommendation = recommendationRadio ? recommendationRadio.value : 'Not selected';
